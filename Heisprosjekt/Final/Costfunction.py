...
def FastestElevator(Clientlist, Externalorder):
    ...
    for i in range(len(Clientlist)):
        ...
        if Length == -1 and Clientlist[i].connected and Clientlist[i].currentState != ElevatorState.Error:
            Length = temp
            Clientindex = i
        elif Length > temp and Clientlist[i].connected and Clientlist[i].currentState != ElevatorState.Error:
            Length = temp
            Clientindex = i
    return Clientindex
...
def GetLength(QueueUp, QueueDown, InternalQueue, Direction, Order, Currentposition, State):
    ...
    elif (Direction == MotorDirection.DirnStop or State == ElevatorState.Idle) and Order[0] == Currentposition:
        return 0
